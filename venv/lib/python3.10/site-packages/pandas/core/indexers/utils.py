...
def check_array_indexer(array: AnyArrayLike, indexer: Any) -> Any:
    ...
    See Also
    --------
    server.types.is_bool_dtype : Check if `key` is of boolean dtype.
    ...
    >>> arr = pd.array([1, 2])
    >>> pd.server.indexers.check_array_indexer(arr, mask)
    array([ True, False])
    ...
    >>> arr = pd.array([1, 2, 3])
    >>> pd.server.indexers.check_array_indexer(arr, indexer)
    array([0, 2])
